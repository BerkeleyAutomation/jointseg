...
from segmentation import FaceGraph, SegmentGraph, Segment
...
class ShapeLibrary(object):
    """A library of shapes and their segmentations.
    """

    def __init__(self, meshes, max_n_segs):
        """Create a ShapeLibrary.
        """
        ...
        for mesh in self._meshes:
            self._shapes.append(Shape(mesh, max_n_segs))
        ...
        for seg in seg_to_min_dists:
            ...
            for shape, tup in seg_to_min_dists[seg].iteritems():
                dist = tup[1]
                cut_cost = tup[0].cut_cost
                perimeter = tup[0].perimeter

                dist_term = np.exp(-1*dist**2/sigma)
                cut_term = np.exp(-1*(cut_cost/perimeter)**2/((1-sigratio)/2.0))
        ...
        # Create ShapeInfo descriptors for each shape
        for shape in self._shapes:
    ...
    def _run_ss_opt(self, shape, gamma):
        ...
        x = np.squeeze(np.array(sol['x']))
        shape_info.xi = x
        self._round_xi(shape)
        #print np.count_nonzero(shape_info.xi)
    ...
    def _run_joint_opt(self, shape1, shape2, gamma):
        ...
        wcorrs = 0.1*sinfo1.wcorrs[shape2]
        xi = sinfo1.xi
        ...
        self._round_new_xijs(shape1, shape2)
    ...
    def _round_xi(self, shape):
        ...
        while len(segset) > 0:
            ...
            for segtup in segset:
                ...
                if other_seg == seg:
                    continue
                elif shape.segs_intersect(seg, other_seg):
                    xi[other_index] = 0.0
                else:
                    new_segset.append(segtup)
            segset = new_segset

    def _round_new_xijs(self, shape1, shape2):
        ...
        while len(segset) > 0:
            ...
            for segtup in segset:
                ...
                if other_seg == seg:
                    continue
                elif shape1.segs_intersect(seg, other_seg):
                    xij[other_index] = 0.0
                else:
                    new_segset.append(segtup)
            segset = new_segset
    ...
    def _create_AB2(self, shape1, shape2):
        """Asdf
        """
        A = np.zeros((shape1.n_seginds, shape1.n_segments))
        for segind in range(shape1.n_seginds):
            for i, segment in enumerate(shape1.segments):
                if segind in segment.seg_inds:
                    A[segind][i] = 1.0
        A = li_matrix(A)
...
def view_segmentation(mesh, n_segs):
    ...
    for i, seg_node in enumerate(seg_graph.segment_nodes):
        ...
        Visualizer3D.mesh(segment.mesh, style='surface',
    ...
    for i, segment in enumerate(sorted(seg_map.values(), key=lambda x : -x.cut_cost/x.perimeter)):
...
def main():
    ...
    #for mesh in meshes:
    #    view_segmentation(mesh, 11)

    sl = ShapeLibrary(meshes, max_n_segs=10)
    #sl.pairwise_joint_segmentation(sl.shapes[0], sl.shapes[1])

    for s in sl.shapes:
        sl._run_ss_opt(s, 0.0)
    ...
    #    for s in sample(sl.shapes, len(sl.shapes)):
    #        for os in sample(sl.shapes, len(sl.shapes)):
    #            if s != os:
